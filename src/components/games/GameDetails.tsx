...
export default function GameDetails({ game }: GameDetailsProps) {
  return (
    <div className="flex items-start justify-between gap-8 pb-6">
      ...
      {game.githubUsername && (
        <div className="flex shrink-0 flex-col items-center gap-2 rounded-2xl border border-zinc-800/50 bg-zinc-900/50 px-6 py-5">
          <ContributorAvatar
            username={game.githubUsername}
            name={game.githubUsername}
            avatarUrl={
              game.urlPhoto ||
              `https://avatars.githubusercontent.com/${game.githubUsername}?s=72`
            }
            size={72}
          />
